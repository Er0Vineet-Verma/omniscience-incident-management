...
// All chips here render once per table row, so they are memoized: list-level
// re-renders (filters, polling refreshes) skip rows whose props are unchanged.

/** Priority chip per the Stitch design: P1 uses the error container, others neutral. */
export const PriorityChip = memo(function PriorityChip({ priority }: { priority: Priority }) {
...
export const StatusText = memo(function StatusText({ status }: { status: IncidentStatus }) {
  return (
...
export const StatusChip = memo(function StatusChip({ status }: { status: IncidentStatus }) {
...
export const PrioritySeverity = memo(function PrioritySeverity({ priority }: { priority: Priority }) {
